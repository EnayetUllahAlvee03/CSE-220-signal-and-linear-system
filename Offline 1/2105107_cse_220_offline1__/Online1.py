...
def main():
    ...
    for i, (impulse, coef) in enumerate(zip(impulses, coeffs)):
        # Create the continuous signal where it equals coef from t = i to t = i + 1
        conti_f = ContinuousSignal(lambda t, c=coef, idx=(i - 6)/2: np.where((t >= idx) & (t <= (idx + 0.5)),  c, 0))
        # Plot the continuous signal
        conti_f.plot(t_range=(-3, 3), title=f"δ(t-{i-6}∇)*({i-6}∇)∇", filename=f"cont_impulse_{i-6}.png")
        # Multiply with the impulse signal and append to the list
        cont_impulse_signals.append(impulse.multiply(conti_f))
