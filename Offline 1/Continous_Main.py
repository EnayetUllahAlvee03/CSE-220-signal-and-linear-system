...
def main():
    ...
    sum_signal.plot(t_range=(-INF, INF), title="Reconstruced", filename="sum_of_impulses reconstruct.png")
    ...
    for i, response in enumerate(response_of_impulses):
        response.plot(t_range=(-INF, INF), title=f"Response of Impulse {i - INF / delta}", filename=f"response_of_impulse_{i - INF / delta}.png")
        # print("i inf delta: ",i,INF,delta)
        # print("i-inf/delta: ",i-INF/delta)  
    output_sig.plot(t_range=(-INF, INF), title="Output Signal", filename="output_signal.png")

    continuous_signal = ContinuousSignal(lambda t: np.where(t >= 0, 1 - np.exp(-t), 0))

    continuous_signal.plot(t_range=(-INF, INF), title="1 - exp(-t)u(t)", filename="1_minus_exp.png")
    lti_continuous = LTI_Continuous(continuous_impulse)
    ...
    for delta in delta_values:
        ...
        output_signal.plot_together(t_range=(-INF, INF), title=f"Sum of impulses for δ={delta} (1 - exp(-t)u(t))", filename=f"Reconstructed_1_minus_exp_{delta}.png", newsig=continuous_signal)
